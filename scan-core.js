...
export function applyAutoContrast(d,w,h){
  ...
  if (hi-lo<30) return;
  // v11.74: REVERTED to the v11.31 curve, byte for byte.
  //
  // v11.73 softened this to map into 6..248, on the reasoning that it was
  // clipping 12.6% of the page to pure white. The measurement was right and the
  // conclusion was wrong: this function is IDENTICAL in v11.31, the build whose
  // scans were good. The clipping was never the problem — it only became
  // visible once the 1-bit stencil was added on top. Tuning it was me changing
